...
vi ArticulationPoints(int v,vi graph[]){
	
	vi ans;
	set<int> temp; // will maintain a set to avoid reduandancy
	...
	for(auto node : temp) ans.push_back(node);
...
int main(){
	...
	vi ans = ArticulationPoints(v,graph);
	for(auto v1 : ans){
		cout<<v1<<" ";
	}
